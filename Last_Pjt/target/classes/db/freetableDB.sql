...
UPDATE FREETABLE SET STEP=STEP+1
WHERE FREETABLE_GROUPNO = (SELECT FREETABLE_GROUPNO FROM FREETABLE WHERE FREETABLE_NO=2) AND 
STEP > (SELECT FREETABLE_STEP FROM FREETABLE WHERE FREETABLE_NO=2);
...
1,FREECOMM_SEQ.NEXTVAL,'관리자','첫번째 글입니다',SYSDATE,
FREECOMM_GROUPNOSEQ.NEXTVAL,1,0
);

INSERT INTO FREECOMM VALUES(
1,1,FREECOMM_SEQ.NEXTVAL,'홍길동','ㄴ첫번째 글의 답글 입니다.',SYSDATE,
(SELECT FREECOMM_GROUPNO FROM MYCOMM WHERE FREECOMM_NO=1),
(SELECT FREECOMM_STEP+1 FROM MYCOMM WHERE FREECOMM_NO=1),
(SELECT FREECOMM_TITLETAB+1 FROM MYCOMM WHERE FREECOMM_NO=1)
);
